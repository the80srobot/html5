...
func TestTextNode(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			input:   &TextNode{Value: FullyTrustedString("Hello, World!"), IndentStyle: Block},
			depth:   1,
		...
		},
		{
			...
			input:   &TextNode{Value: FullyTrustedString("Hello, World!"), IndentStyle: Block, Width: 1},
			depth:   1,
			opts:    &CompileOptions{Indent: "  "},
			output:  "\n  Hello,\n  World!",
		},
		{
			comment: "binding",
			input:   &TextNode{Value: Bind("hello")},
			opts:    &CompileOptions{},
